...
class ModeledStreamUtility(object):
    ...
    @staticmethod
    def load_run_and_attach_gain(runfile, updlens, matches, useAverageLengths):
        run = {}
    ...
    def sample_users_from_population(self):
    ...
    def compute_population_MSU(self, run):
        """
        ...
        for qid in sorted(run.keys()):
            ...
            topic_updates.sort(key=lambda t: t.conf)
            topic_updates.sort(key=lambda t: t.time)
...
class MSUReverseChronoOrder(ModeledStreamUtility, \
    ...
    def _compute_user_MSU(self, user_instance, updates):
        ...
        # TODO: load the query durations from file
        #ssn_durns = []
        #away_durns = []
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="computes Modeled Stream Utility for system, using pre-generated time-trails")
    ...
    ap.add_argument("time_trail_folder", help="folder containing (session, away) times for each simulated user (one file per user; one line per (session, away) times)")
    ...
    logger.warning('ONLY WORKS WITH Temporal Summarization Track 2013')
    ...
    updlens = utils.read_in_update_lengths(args.update_lengths_folder)    
    ...
    for runfile in args.runfiles:
        ...
        try: 
            run = MSU.load_run_and_attach_gain(runfile, updlens, matches, False)
            ignored_qid = '7'
        ...
        run_msu = MSU.compute_population_MSU(run)
